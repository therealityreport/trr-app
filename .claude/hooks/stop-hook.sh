...
if [ "$CHANGES_DETECTED" = true ]; then
  ...
  echo ""
  echo "  1️⃣  Validate your work:"
  echo "      make validate"
  echo "      (or: cd apps/web && npm run lint && npx tsc --noEmit && npm run test)"
  echo ""
  echo "  2️⃣  Review changes:"
  echo "      git status"
  echo "      git diff"
  echo ""
  echo "  3️⃣  If validation passes, commit:"
  echo "      git add ."
  echo "      git commit -m \"your message\""
  echo ""
  echo "  4️⃣  Or continue working in your next session"
  echo ""
else
  ...
  if [ "$COMMITS_AHEAD" -gt 0 ]; then
    ...
    echo ""
    echo "💡 Ready to start new work or switch branches"
    echo ""
  fi
fi
