...
export default function BravodleCover() {
  ...
  const displayDate = useMemo(() => formatBravodleDisplayDate(), []);
  ...
  const displayMessage = errorMessage ?? (!manager ? BRAVODLE_UNAVAILABLE_MESSAGE : null);
  ...
  useEffect(() => {
    ...
    const bootstrap = async () => {
      try {
        AuthDebugger.log("Bravodle Cover: Bootstrapping game state", {
          uid: userId,
          puzzleDateKey,
        });
        ...
        AuthDebugger.log("Bravodle Cover: Failed to bootstrap game", { message });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        AuthDebugger.log("Bravodle Cover: Unable to compute puzzle number", {
  ...
  useEffect(() => {
    AuthDebugger.log("Bravodle Cover: Component mounted");
    
    const unsubscribe = auth.onAuthStateChanged((authUser) => {
      AuthDebugger.log("Bravodle Cover: Auth state changed", { 
        hasUser: !!authUser,
    ...
    return () => {
      AuthDebugger.log("Bravodle Cover: Component unmounting");
      unsubscribe();
    };
  }, []);
  ...
  const handlePrimaryCta = async () => {
    if (!manager) {
      setErrorMessage(BRAVODLE_UNAVAILABLE_MESSAGE);
      return;
    }

    if (!userId) {
      AuthDebugger.log("Bravodle Cover: No user logged in, redirecting to register");
      router.push("/auth/register");
      return;
    }

    if (cta === "stats") {
      AuthDebugger.log("Bravodle Cover: CTA 'See Stats' selected, navigating to stats", {
        uid: userId,
      });
      router.push("/bravodle/play?show=stats");
      return;
    }
    ...
    if (cta === "start") {
      try {
        ...
        AuthDebugger.log("Bravodle Cover: Failed to mark game as started", { message });
      }
    }

    AuthDebugger.log("Bravodle Cover: CTA pressed, navigating to play", {
      uid: userId,
      cta,
    });
    router.push("/bravodle/play");
  };

  const handleBack = () => {
    AuthDebugger.log("Bravodle Cover: Back button clicked, going back");
    router.back();
  };
  ...
  return (
    <div className="min-h-screen bg-[#d0adc9]">
