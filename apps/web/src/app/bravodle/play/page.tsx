...
} from "@/lib/bravodle/types";
import { getBravodleDateKey } from "@/lib/bravodle/utils";
...
const BRAVODLE_UNAVAILABLE_MESSAGE =
  "Bravodle is temporarily unavailable because Firebase Firestore is not configured in this environment.";
...
const BOARD_ROW_GAP_PX = 5; // vertical spacing between board rows
...
const getCellId = (rowIndex: number, columnKey: BravodleBoardColumnKey): string => `r${rowIndex}-c${columnKey}`;
...
function resolveFieldDisplay(field: BravodleGuessField | undefined | null): {
        ...
        return (
          <div className="space-y-3 text-sm leading-relaxed text-black">
            ...
            <ul className="list-disc space-y-1 pl-4 font-['HamburgSerial']">
              ...
              <li>
                <span className="font-semibold text-[#644073]">Purple</span> = same season together, but on a
                non-Bravo show.
              </li>
              <li>
                <span className="font-semibold">Yellow</span> = same show, different seasons.
              </li>
              <li>
                <span className="font-semibold">Gray</span> = no shared show yet.
              </li>
            </ul>
          </div>
        );
        ...
        return (
          <div className="space-y-3 text-sm leading-relaxed text-black">
            ...
            <ul className="list-disc space-y-1 pl-4 font-['HamburgSerial']">
              ...
              <li>
                <span className="font-semibold">Yellow</span> = within five episodes.
              </li>
              <li>
                <span className="font-semibold">Gray</span> = no data or significant difference.
              </li>
            </ul>
            <p className="text-xs text-gray-500">
              Episode totals include Bravo seasons we can confirm from official sources.
            </p>
          </div>
        );
      case "wwhl":
...
export default function BravodleGamePage() {
  const router = useRouter();
  const manager = useBravodleManager();
  const [authLoading, setAuthLoading] = useState(true);
  const [user, setUser] = useState<User | null>(null);
  const [gameSnapshot, setGameSnapshot] = useState<BravodleGameSnapshot | null>(null);
  const [isBootstrapping, setIsBootstrapping] = useState(false);
  const [errorMessage, setErrorMessage] = useState<string | null>(null);
  const [talentQuery, setTalentQuery] = useState("");
  const [talentIndex, setTalentIndex] = useState<BravodleTalentRecord[] | null>(null);
  const [talentLoading, setTalentLoading] = useState(false);
  const [talentError, setTalentError] = useState<string | null>(null);
  const [selectedTalent, setSelectedTalent] = useState<BravodleTalentRecord | null>(null);
  ...
  const [ageOrZodiac, setAgeOrZodiac] = useState<"age" | "zodiac">(
    () => (typeof window !== "undefined" ? (localStorage.getItem("bravodle:ageOrZodiac") as "age" | "zodiac") : null) || "age",
  );
  useEffect(() => { try { localStorage.setItem("bravodle:ageOrZodiac", ageOrZodiac); } catch {} }, [ageOrZodiac]);
  useEffect(() => { if (!user?.uid) return; (async () => { try { const prefs = await getUserPreferences(user.uid); if (prefs?.bravodle?.ageOrZodiac) setAgeOrZodiac(prefs.bravodle.ageOrZodiac); } catch {} })(); }, [user?.uid]);
  const [settingsOpen, setSettingsOpen] = useState(false);
  const [completionStats, setCompletionStats] = useState<BravodleStatsSummary | null>(null);
  ...
  const activeColumns = useMemo(
    () => [
      { key: "guess" as BravodleBoardColumnKey, label: "NAME" },
      { key: "gender" as BravodleBoardColumnKey, label: "GENDER" },
      { key: (ageOrZodiac === "age" ? "age" : "zodiac") as BravodleBoardColumnKey, label: ageOrZodiac === "age" ? "AGE" : "ZODIAC" },
      { key: "shows" as BravodleBoardColumnKey, label: "SHOWS" },
      { key: "episodes" as BravodleBoardColumnKey, label: "EPISODES" },
      { key: "wwhl" as BravodleBoardColumnKey, label: "WWHL" },
    ],
    [ageOrZodiac],
  );
  ...
  const handleFeedback = useCallback(() => {
    ...
    const target = BRAVODLE_FEEDBACK_TARGET;
    if (!target) return;
  ...
  // handleShareResults is defined after activeColumns so it can use the current column set

  useEffect(() => {
    AuthDebugger.log("Bravodle Game: Component mounted");
    const unsubscribe = auth.onAuthStateChanged((authUser) => {
      AuthDebugger.log("Bravodle Game: Auth state changed", {
        hasUser: !!authUser,
    ...
    return () => {
      AuthDebugger.log("Bravodle Game: Component unmounting");
      unsubscribe();
    };
  }, []);
  ...
  useEffect(() => {
    if (!authLoading && !userId) {
      AuthDebugger.log("Bravodle Game: No user, redirecting to register");
  ...
  useEffect(() => {
    ...
    const bootstrap = async () => {
      ...
      try {
        AuthDebugger.log("Bravodle Game: Starting bootstrap", {
          uid: userId,
          puzzleDateKey,
        });
        ...
        AuthDebugger.log("Bravodle Game: Bootstrap failed", { message });
  ...
  useEffect(() => {
    ...
    if (!activeManager) {
      setTalentIndex(null);
      setTalentError(BRAVODLE_UNAVAILABLE_MESSAGE);
    ...
    activeManager
      ...
      .catch((error) => {
        ...
        AuthDebugger.log("Bravodle Game: Talent index fetch failed", { message });
  ...
  const isTalentEligible = useCallback((talent: BravodleTalentRecord | null | undefined): boolean => {
    if (!talent) return false;
    const metadata = (talent.metadata ?? {}) as Record<string, unknown>;
    ...
    const normalizeList = (source: unknown): string[] => {
      if (!source) return [];
    ...
    const explicitFlag = toBoolean(metadata.__bravodleEligible);
    if (explicitFlag !== null) return explicitFlag;

    const directFlag = toBoolean(
      metadata.bravodle ?? metadata.Bravodle ?? metadata.bravodleEligible ?? metadata.BravodleEligible,
    );
    ...
    const gameSources = [
      ...
      metadata.gameModes,
      metadata.GameModes,
      metadata.availableGames,
    ];

    return gameSources.some((source) =>
      normalizeList(source).some((entry) => entry.toLowerCase() === "bravodle"),
    );
  }, []);

  const eligibleTalentIndex = useMemo(() => {
    if (!talentIndex) return [] as BravodleTalentRecord[];
    return talentIndex.filter((talent) => isTalentEligible(talent));
  }, [isTalentEligible, talentIndex]);

  const findTalentByQuery = useCallback(
    (value: string): BravodleTalentRecord | null => {
      if (!eligibleTalentIndex.length) return null;
      const normalized = value.trim().toLowerCase();
      if (!normalized) return null;

      for (const talent of eligibleTalentIndex) {
  ...
  const applyQueryValue = useCallback(
    (
      nextValue: string,
      options: { openDropdown?: boolean; selectedTalent?: BravodleTalentRecord | null } = {},
    ) => {
  ...
  const talentMatches = useMemo(() => {
    ...
    if (query.length < 3 || !eligibleTalentIndex.length) {
      return [] as Array<{ talent: BravodleTalentRecord; matchedAlternative?: string }>;
    }

    const matches: Array<{ talent: BravodleTalentRecord; matchedAlternative?: string }> = [];
    ...
    return matches;
  }, [eligibleTalentIndex, talentQuery]);

  const orderedGuesses = useMemo(() => {
    if (!gameSnapshot?.guesses) return [] as BravodleGuess[];
  ...
  const blockingErrorMessage = errorMessage ?? (!manager ? BRAVODLE_UNAVAILABLE_MESSAGE : null);

  if (authLoading || (isBootstrapping && !gameSnapshot)) {
    return (
      <div className="min-h-screen bg-blue-300 flex items-center justify-center">
        <p className="text-black">Loading Bravodle...</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-blue-300 flex items-center justify-center px-4">
        <div className="max-w-md w-full bg-white p-8 rounded-lg shadow-lg text-center space-y-4">
          ...
          <button
            onClick={() => router.push("/bravodle/cover")}
  ...
  return (
    <div className="min-h-screen bg-white text-black bravodle">
      <div className="mx-auto flex min-h-screen w-full max-w-[1440px] flex-col">
        ...
        <main className="flex flex-1 justify-center px-4 pb-12 pt-10">
          <div className="flex w-full max-w-[480px] flex-col items-center gap-8">
            ...
            <div id="bravodle-keyboard" className="w-full flex justify-center">
...
interface TalentSearchProps {
  ...
  matches: Array<{ talent: BravodleTalentRecord; matchedAlternative?: string }>;
  loading: boolean;
  ...
  onSelect: (talent: BravodleTalentRecord) => void;
  onSubmit: () => void;
                  ...
                  {matches.map(({ talent }) => (
                    <li key={talent.id}>
                      ...
                      >
                        ...
                        {/* Hide the "matches alternative" note but keep logic for matching */}
                      </button>
                    </li>
                  ))}
                </ul>
...
function BravodleBoard({
  ...
  columns: Array<{ key: BravodleBoardColumnKey; label: string }>;
}) {
  const rows = useMemo(() => {
    const rowCount = Math.max(BOARD_MIN_ROWS, Math.min(BOARD_ROWS, (guesses?.length ?? 0) + 1));
    const grid: Array<BravodleGuess | null> = Array.from({ length: rowCount }, () => null);
  ...
  useEffect(() => {
    ...
    const computeSize = () => {
      ...
      const keyboardEl = document.getElementById("bravodle-keyboard");
  ...
  // (share logic is passed inline to CompletedView to avoid scope issues)
...
function KeyboardKey({
  ...
  const baseClasses = `flex ${sizeClasses} items-center justify-center rounded-sm bg-[#d0adc9] uppercase text-black transition`;
...
function BoardCell({
  ...
  frontValue: string;
  backValue: string;
  backVerdict: BravodleGuessVerdict;
  label: string;
  guessNumber: number;
  isGuessColumn: boolean;
  columnKey: BravodleBoardColumnKey;
  ...
  const winningClasses = "bg-[#4d2e43] text-white border-[#4d2e43]";
