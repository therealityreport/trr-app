...
const getErrorDetail = (error: unknown): string => {
  ...
  if (error instanceof Error) {
    if (error.name === "AbortError") {
      return "Timed out waiting for backend reprocess stream response.";
    }
 ...
 *
 * Proxies reprocess (metadata repair + count + text-ID + crop + resize) request to TRR-Backend with SSE streaming.
 */
  ...
  try {
    ...
    const stream = new ReadableStream<Uint8Array>({
      async start(controller) {
        ...
        emitProxyProgress({
          message: "Backend health check passed. Connecting to backend reprocess stream...",
          current: 0,
          total: BACKEND_FETCH_ATTEMPTS,
          attempt: 0,
          max_attempts: BACKEND_FETCH_ATTEMPTS,
        ...
        emitProxyProgress({
          message: "Connecting to backend reprocess stream...",
          current: 0,
          total: BACKEND_FETCH_ATTEMPTS,
          attempt: 0,
          max_attempts: BACKEND_FETCH_ATTEMPTS,
        ...
        for (let attempt = 1; attempt <= BACKEND_FETCH_ATTEMPTS; attempt += 1) {
          ...
          emitProxyProgress({
            message:
              attempt === 1
                ? "Connecting to backend reprocess stream..."
        ...
        emitProxyProgress({
          message: "Connected to backend reprocess stream.",
        ...
        try {
          ...
          if (sawBackendComplete && isSocketTerminationError(error)) {
            // Undici can raise socket termination while closing after a complete event.
          } else {
          emitEvent("error", {
            stage: "proxy_stream",
