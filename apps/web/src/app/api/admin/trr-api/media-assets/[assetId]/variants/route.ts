 ...
 *
 * Proxy to TRR-Backend variants endpoint for media assets.
 */
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const backendUrl = getBackendApiUrl(`/admin/media-assets/${assetId}/variants`);
    if (!backendUrl) {
    ...
    if (!backendResponse.ok) {
      const errorMessage =
        typeof data.error === "string" ? data.error : "Variant generation failed";
    ...
    console.error("[api] Failed to generate media asset variants", error);
