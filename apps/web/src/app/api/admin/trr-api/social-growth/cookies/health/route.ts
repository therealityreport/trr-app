...
const SOCIALBLADE_COOKIE_HEALTH_TIMEOUT_MS = 50_000;
...
export async function GET(request: NextRequest) {
  try {
    ...
    const backendUrl = getBackendApiUrl(`/admin/people/socialblade/cookies/health${query ? `?${query}` : ""}`);
    if (!backendUrl) {
    ...
    const upstream = await timeoutSafeFetch(backendUrl, {
      headers,
      timeoutMs: SOCIALBLADE_COOKIE_HEALTH_TIMEOUT_MS,
    ...
    console.error("[api] Failed to get SocialBlade cookie health", error);
