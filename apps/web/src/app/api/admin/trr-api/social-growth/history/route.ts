...
const SOCIALBLADE_HISTORY_TIMEOUT_MS = 20_000;
...
export async function GET(request: NextRequest) {
  try {
    ...
    const backendUrl = getBackendApiUrl(`/admin/people/socialblade/history${query ? `?${query}` : ""}`);
    if (!backendUrl) {
    ...
    const upstream = await timeoutSafeFetch(backendUrl, {
      headers,
      timeoutMs: SOCIALBLADE_HISTORY_TIMEOUT_MS,
      timeoutName: "socialblade-history",
    });
    ...
    if (isTimeoutSafeFetchTimeoutError(error)) {
      return buildSocialBladeTimeoutResponse(error, SOCIALBLADE_HISTORY_TIMEOUT_MS);
    }
    console.error("[api] Failed to get SocialBlade history", error);
