...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const data = await fetchSeasonBackendJson(
      showId,
      seasonNumber,
      `/tiktok/sounds/${encodeURIComponent(soundId)}/posts`,
      {
        queryString: forwardedSearchParams.toString(),
        seasonIdHint,
        fallbackError: "Failed to fetch TikTok sound posts",
        retries: 1,
        timeoutMs: 30_000,
      },
    );
    ...
    return socialProxyErrorResponse(error, "[api] Failed to fetch TikTok sound posts");
  }
}
