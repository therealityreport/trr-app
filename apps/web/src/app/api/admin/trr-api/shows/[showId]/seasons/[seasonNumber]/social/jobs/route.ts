...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const backendBase = getBackendApiUrl(`/admin/socials/seasons/${seasonId}/ingest/jobs`);
    if (!backendBase) {
      ...
      const error =
        typeof data.error === "string"
          ...
          : typeof data.detail === "string"
            ? data.detail
            : "Failed to fetch social jobs";
      return NextResponse.json({ error }, { status: response.status });
    }

    return NextResponse.json(data);
  } catch (error) {
    console.error("[api] Failed to fetch social jobs", error);
