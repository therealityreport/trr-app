...
const SOCIAL_RUNS_CACHE_NAMESPACE = "admin-social-runs";
const SOCIAL_RUNS_CACHE_TTL_MS = parseCacheTtlMs(
  process.env.TRR_ADMIN_SOCIAL_RUNS_CACHE_TTL_MS,
);
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const cacheKey = buildUserScopedRouteCacheKey(
      user.uid,
      `${showId}:${seasonNumber}:runs`,
      forwardedSearchParams,
    );
    const cachedData = getRouteResponseCache<unknown>(SOCIAL_RUNS_CACHE_NAMESPACE, cacheKey);
    if (cachedData) {
    ...
    setRouteResponseCache(SOCIAL_RUNS_CACHE_NAMESPACE, cacheKey, data, SOCIAL_RUNS_CACHE_TTL_MS);
