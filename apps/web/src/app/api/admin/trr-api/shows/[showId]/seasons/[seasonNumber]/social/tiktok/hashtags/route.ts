...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const data = await fetchSeasonBackendJson(showId, seasonNumber, "/tiktok/hashtags", {
      queryString: forwardedSearchParams.toString(),
      seasonIdHint,
      fallbackError: "Failed to fetch TikTok hashtags",
      retries: 1,
      timeoutMs: 25_000,
    });
    return NextResponse.json(data);
  } catch (error) {
    return socialProxyErrorResponse(error, "[api] Failed to fetch TikTok hashtags");
  }
}
