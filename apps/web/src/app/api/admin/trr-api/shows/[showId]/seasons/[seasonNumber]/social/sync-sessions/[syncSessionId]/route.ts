...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const data = await fetchSeasonBackendJson(showId, seasonNumber, `/sync-sessions/${syncSessionId}`, {
      queryString: forwardedSearchParams.toString(),
      seasonIdHint,
      fallbackError: "Failed to fetch social sync session",
      retries: 0,
    ...
    return socialProxyErrorResponse(error, "[api] Failed to fetch social sync session");
  }
}
