...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const data = await fetchSeasonBackendJson(showId, seasonNumber, `/sync-sessions/${syncSessionId}/retry`, {
      method: "POST",
      body: requestText || JSON.stringify({}),
    ...
    return socialProxyErrorResponse(error, "[api] Failed to retry social sync session");
  }
}
