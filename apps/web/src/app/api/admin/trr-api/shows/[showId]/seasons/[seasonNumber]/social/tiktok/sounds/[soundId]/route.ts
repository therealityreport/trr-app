...
interface RouteParams {
  params: Promise<{ showId: string; seasonNumber: string; soundId: string }>;
}

export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    await requireAdmin(request);
    const { showId, seasonNumber, soundId } = await params;
    ...
    if (!soundId?.trim()) {
      return NextResponse.json({ error: "soundId is required", code: "BAD_REQUEST", retryable: false }, { status: 400 });
    }
    ...
    const data = await fetchSeasonBackendJson(showId, seasonNumber, `/tiktok/sounds/${encodeURIComponent(soundId)}`, {
      queryString: forwardedSearchParams.toString(),
      seasonIdHint,
      fallbackError: "Failed to fetch TikTok sound detail",
      retries: 1,
      timeoutMs: 25_000,
    });
    return NextResponse.json(data);
  } catch (error) {
    return socialProxyErrorResponse(error, "[api] Failed to fetch TikTok sound detail");
  }
}
