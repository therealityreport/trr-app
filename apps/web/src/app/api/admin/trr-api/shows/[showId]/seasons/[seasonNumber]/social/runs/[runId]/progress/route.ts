...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const data = await fetchSeasonBackendJson(showId, seasonNumber, `/ingest/runs/${runId}/progress`, {
      ...
      fallbackError: "Failed to fetch social run progress",
      retries: 0,
    ...
    return socialProxyErrorResponse(error, "[api] Failed to fetch social run progress");
  }
}
