...
const getErrorDetail = (error: unknown): string => {
  ...
  if (error instanceof Error) {
    if (error.name === "AbortError") {
      return "Timed out waiting for backend batch-jobs stream response.";
    }
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const backendUrl = getBackendApiUrl(`/admin/shows/${showId}/assets/batch-jobs/stream`);
    if (!backendUrl) {
      return Response.json(
        {
          stage: "proxy",
    ...
    const stream = new ReadableStream<Uint8Array>({
      async start(controller) {
        ...
        emitProxyProgress({
          message: "Connecting to backend batch-jobs stream...",
          current: 0,
          total: BACKEND_FETCH_ATTEMPTS,
          attempt: 0,
          max_attempts: BACKEND_FETCH_ATTEMPTS,
        ...
        for (let attempt = 1; attempt <= BACKEND_FETCH_ATTEMPTS; attempt += 1) {
          ...
          emitProxyProgress({
            message:
              attempt === 1
                ? "Connecting to backend batch-jobs stream..."
        ...
        emitProxyProgress({
          message: "Connected to backend batch-jobs stream.",
