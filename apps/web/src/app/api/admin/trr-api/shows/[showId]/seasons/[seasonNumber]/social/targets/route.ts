...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const backendBase = getBackendApiUrl(`/admin/socials/seasons/${seasonId}/targets`);
    if (!backendBase) {
      ...
      const error =
        typeof data.error === "string"
          ...
          : typeof data.detail === "string"
            ? data.detail
            : "Failed to fetch social targets";
      return NextResponse.json({ error }, { status: response.status });
    }

    return NextResponse.json(data);
  } catch (error) {
    console.error("[api] Failed to fetch social targets", error);
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const seasonId = await resolveSeasonId(showId, seasonNumber);
    const backendUrl = getBackendApiUrl(`/admin/socials/seasons/${seasonId}/targets`);
    if (!backendUrl) {
      return NextResponse.json({ error: "Backend API not configured" }, { status: 500 });
    }
    ...
    if (!response.ok) {
      const error =
        typeof data.error === "string"
          ...
          : typeof data.detail === "string"
            ? data.detail
            : "Failed to update social targets";
      return NextResponse.json({ error }, { status: response.status });
    }

    return NextResponse.json(data);
  } catch (error) {
    console.error("[api] Failed to update social targets", error);
    const message = error instanceof Error ? error.message : "failed";
    const status =
      message === "unauthorized"
        ? 401
