...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const data = await fetchSeasonBackendJson(showId, seasonNumber, "/sync-sessions", {
      method: "POST",
      headers: {
      ...
      body: outboundBody,
      seasonIdHint,
      fallbackError: "Failed to create social sync session",
      retries: 0,
      timeoutMs: 210_000,
    });
    return NextResponse.json(data);
  } catch (error) {
    return socialProxyErrorResponse(error, "[api] Failed to create social sync session");
  }
}
