...
interface RouteParams {
  params: Promise<{ showId: string; seasonNumber: string; postId: string }>;
}

export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    await requireAdmin(request);
    const { showId, seasonNumber, postId } = await params;
    ...
    if (!postId?.trim()) {
      return NextResponse.json({ error: "postId is required", code: "BAD_REQUEST", retryable: false }, { status: 400 });
    }
    ...
    const data = await fetchSeasonBackendJson(showId, seasonNumber, `/tiktok/posts/${encodeURIComponent(postId)}/detail`, {
      queryString: forwardedSearchParams.toString(),
      seasonIdHint,
      fallbackError: "Failed to fetch TikTok post detail",
      retries: 1,
      timeoutMs: 30_000,
    });
    return NextResponse.json(data);
  } catch (error) {
    return socialProxyErrorResponse(error, "[api] Failed to fetch TikTok post detail");
  }
}
