...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const data = await fetchSocialBackendJson(
      `/profiles/${encodeURIComponent(platform)}/${encodeURIComponent(handle)}/comments/runs/${encodeURIComponent(runId)}/cancel`,
      {
        method: "POST",
    ...
    return socialProxyErrorResponse(error, "[api] Failed to cancel social account comments run");
  }
}
