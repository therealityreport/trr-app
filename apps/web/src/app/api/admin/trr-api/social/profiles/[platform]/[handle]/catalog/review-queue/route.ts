...
export async function GET(request: NextRequest, context: RouteContext) {
  try {
    ...
    const data = await fetchSocialBackendJson(
      `/profiles/${encodeURIComponent(platform)}/${encodeURIComponent(handle)}/catalog/review-queue`,
      {
    ...
    return socialProxyErrorResponse(error, "[api] Failed to fetch social account catalog review queue");
  }
}
