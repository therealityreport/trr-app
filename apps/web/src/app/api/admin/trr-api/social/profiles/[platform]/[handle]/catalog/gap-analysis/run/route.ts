...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    const data = await fetchSocialBackendJson(
      `/profiles/${encodeURIComponent(platform)}/${encodeURIComponent(handle)}/catalog/gap-analysis/run`,
      {
        method: "POST",
    ...
    return socialProxyErrorResponse(error, "[api] Failed to start social account catalog gap analysis");
  }
}
