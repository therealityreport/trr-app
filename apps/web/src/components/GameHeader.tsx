...
const DEFAULT_MENU_ID = "header-menu-button";
...
export default function GameHeader({
  ...
  const helpMenuItems = useMemo(() => {
    ...
    if (onHowToPlay) {
      items.push({ label: "How to Play", action: onHowToPlay });
    }
    if (onFeedback) {
      items.push({ label: "Feedback", action: onFeedback });
    }
    if (onReportProblem) {
      items.push({ label: "Report a Problem", action: onReportProblem });
    }
    return items;
  }, [onHowToPlay, onFeedback, onReportProblem]);
  ...
  useEffect(() => {
    ...
    const handlePointerDown = (event: PointerEvent) => {
      ...
      if (helpToggleRef.current && helpToggleRef.current.contains(target)) {
        return;
      }
  ...
  const shouldShowStats = showStatsButton && Boolean(onStatsClick);
  const shouldShowHelp = showHelpMenu && helpMenuItems.length > 0;
  const shouldShowSettings = showSettingsButton !== false;

  const ariaLabel =
    menuButtonAriaLabel ??
    (pathname === "/hub" ? "Open navigation menu" : "Open game navigation menu");

  return (
    <header className="flex h-20 w-full items-center justify-between border-b border-black px-3 pr-6">
        ...
        {shouldShowHelp && (
          <div className="relative" ref={helpMenuRef}>
            ...
            {isHelpMenuOpen && (
              ...
              >
                {helpMenuItems.map(({ label, action }) => (
                  <button
                    key={label}
                    type="button"
                    role="menuitem"
