...
function collectRankingFontOverrides(
  config: CircleRankingConfig | RectangleRankingConfig,
): RankTemplateFontOverrides {
...
export default function RankOrderInput({
  ...
  // Both ranking templates now use slot-grid interactions (unranked tray + ranked slots).
  const variant = "grid";
  const layoutPreset = config.uiVariant === "rectangle-ranking"
    ...
    <div
      className={
        layoutPreset === "figma-rank-circles"
          ? "rounded-[18px] bg-[#D9D9D9] px-2.5 py-3.5"
          : ""
      }
    >
      <FlashbackRanker
    ...
    <div className="space-y-3">
      {fontOverrides.missingFonts.length > 0 && (
        <p
          ...
          data-testid="rank-order-missing-fonts"
        >
      ...
      {fontLoadFailures.length > 0 && (
        <p
          className="rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 text-xs text-amber-900"
          data-testid="rank-order-font-load-failures"
        >
