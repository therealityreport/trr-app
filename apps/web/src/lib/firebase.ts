...
// Sign in with Google (with error filtering for popup cancellations)
export async function signInWithGoogle(): Promise<void> {
  try {
    ...
    // Otherwise, silently ignore popup cancellations
  }
}
...
export async function initAnalytics(): Promise<Analytics | null> {
  try {
    if (analyticsInstance === null && typeof window !== "undefined" && (await isSupported())) {
