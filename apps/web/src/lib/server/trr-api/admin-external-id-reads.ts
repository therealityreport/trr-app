...
const requestedIdByCaseInsensitiveKey = (values: readonly string[]): Map<string, string> =>
...
export async function listPrimaryPersonExternalIdsByPersonIds(
  ...
  const requestedPersonIdByKey = requestedIdByCaseInsensitiveKey(uniquePersonIds);
  ...
  for (const response of responses) {
    for (const person of response) {
      const requestedPersonId = requestedPersonIdByKey.get(person.person_id.toLowerCase());
      if (!requestedPersonId) throw invalidBackendResponse();
      recordsByPersonId.set(requestedPersonId, person.external_ids);
    }
  }
...
export async function listShowExternalIdsByIds(
  ...
  const requestedShowIdByKey = requestedIdByCaseInsensitiveKey(uniqueShowIds);
  ...
  for (const response of responses) {
    for (const show of response) {
      const requestedShowId = requestedShowIdByKey.get(show.show_id.toLowerCase());
      if (!requestedShowId) throw invalidBackendResponse();
      externalIdsByShowId.set(requestedShowId, show.external_ids);
    }
  }
