...
import { GET } from "@/app/api/admin/trr-api/shows/[showId]/seasons/[seasonNumber]/social/analytics/comments-coverage/route";

describe("social comments-coverage proxy route", () => {
  ...
  beforeEach(() => {
    ...
    fetchSeasonBackendJsonMock.mockResolvedValue({ up_to_date: true, total_saved_comments: 0 });
  ...
  it("forwards validated comments coverage query with the default social timeout profile", async () => {
    const request = new NextRequest(
      `http://localhost/api/admin/trr-api/shows/${showId}/seasons/6/social/analytics/comments-coverage?season_id=${seasonId}&source_scope=bravo&platforms=instagram&date_start=2026-01-01T00:00:00.000Z&date_end=2026-01-08T00:00:00.000Z`,
      { method: "GET" },
    );
    ...
    expect(fetchSeasonBackendJsonMock.mock.calls[0]?.[3]?.queryString).toContain(
      "date_start=2026-01-01T00%3A00%3A00.000Z",
    );
  });

  it("returns 400 for invalid season_id hint", async () => {
    const request = new NextRequest(
      `http://localhost/api/admin/trr-api/shows/${showId}/seasons/6/social/analytics/comments-coverage?season_id=bad-season-id`,
      { method: "GET" },
    );
