...
describe("public identity API contract", () => {
  ...
  it("accepts an omitted optional person show context as null", async () => {
    const { show_context: _showContext, ...responseWithoutContext } = PERSON_RESPONSE;
    timeoutSafeFetchMock.mockResolvedValue(Response.json(responseWithoutContext));

    await expect(resolvePublicPersonIdentity("alex-smith")).resolves.toMatchObject({
      show_context: null,
    });
  });
