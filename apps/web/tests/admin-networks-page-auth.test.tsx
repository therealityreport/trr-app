...
describe("Admin networks page auth + sync UI", () => {
  ...
  it("sends bearer headers and renders unresolved sync details", async () => {
    ...
    expect(screen.getByRole("button", { name: "Hide unresolved logos" })).toBeInTheDocument();
    expect(screen.getAllByText("Bravo").length).toBeGreaterThan(0);
    expect(screen.getByText(/Showing first 300 unresolved entries/)).toBeInTheDocument();
  });
