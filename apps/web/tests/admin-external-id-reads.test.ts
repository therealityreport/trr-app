...
describe("admin external-ID v2 reads", () => {
  ...
  it("preserves requested mixed-case UUID keys when the backend normalizes response casing", async () => {
    const requestedPersonId = PERSON_A.toUpperCase();
    const requestedShowId = PERSON_B.toUpperCase();
    fetchAdminBackendJsonMock
      .mockResolvedValueOnce({
        status: 200,
        data: {
          people: [{ person_id: PERSON_A, external_ids: [externalIdRecord()] }],
        },
      })
      .mockResolvedValueOnce({
        status: 200,
        data: {
          shows: [{ show_id: PERSON_B, external_ids: { imdb: "tt1234567" } }],
        },
      });

    const people = await listPrimaryPersonExternalIdsByPersonIds([
      requestedPersonId,
      PERSON_A,
    ]);
    const shows = await listShowExternalIdsByIds([requestedShowId]);

    expect([...people.keys()]).toEqual([requestedPersonId]);
    expect(people.get(requestedPersonId)).toEqual([externalIdRecord()]);
    expect([...shows.keys()]).toEqual([requestedShowId]);
    expect(shows.get(requestedShowId)).toEqual({ imdb: "tt1234567" });
  });
