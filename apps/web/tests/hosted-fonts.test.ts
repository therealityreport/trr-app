...
describe("hosted font helpers", () => {
  ...
  it("builds upstream R2 URLs from the shared hosted font base", () => {
    ...
    expect(preloadUrl).toBe(
      "https://fonts.example.test/fonts/trr/Hamburg%20Serial/HamburgSerial-930108065.otf",
    );
  });
  ...
  it("builds same-origin asset paths for runtime preloads", () => {
    expect(buildHostedFontAssetPath(HOSTED_FONT_PRELOADS[1]?.assetPath ?? "")).toBe(
      "/fonts/trr/Gloucester/GloucesterOldStyle-5735713.ttf",
    );
  });
  ...
  it("rewrites absolute hosted font templates to same-origin hosted font asset paths", () => {
    const css = buildHostedFontsStylesheetFromTemplates(
      [
        `@font-face { src: url("https://fonts.legacy.example/fonts/trr/Hamburg%20Serial/HamburgSerial-930108065.otf") format("opentype"); }`,
        `@font-face { src: url("${DEFAULT_HOSTED_FONT_BASE_URL}/fonts/reference%20fonts/NYTimes/NYTKarnak_Condensed.woff2") format("woff2"); }`,
      ],
      "https://fonts.example.test/",
    );

    expect(css).toContain(
      'url("/fonts/trr/Hamburg%20Serial/HamburgSerial-930108065.otf")',
    );
    expect(css).toContain(
      'url("/fonts/reference%20fonts/NYTimes/NYTKarnak_Condensed.woff2")',
    );
  ...
  it("extracts concrete upstream font file links from hosted stylesheet css", () => {
    const fontAssetLinks = extractHostedFontAssetLinks(
      [
        '@font-face { font-family: "Beton"; src: url("/fonts/trr/Beton/Beton%20T%20Extended%20Bold.ttf") format("truetype"); }',
        '@font-face { font-family: "Biotif Pro"; src: url("https://fonts.example.test/fonts/trr/Biotif%20Pro/BiotifProBook-930829391.otf") format("opentype"); }',
      ].join("\n"),
      "https://fonts.example.test/",
    );

    expect(fontAssetLinks["Beton"]).toBe(
      "https://fonts.example.test/fonts/trr/Beton/Beton%20T%20Extended%20Bold.ttf",
    );
    expect(fontAssetLinks["Biotif Pro"]).toBe(
      "https://fonts.example.test/fonts/trr/Biotif%20Pro/BiotifProBook-930829391.otf",
    );
  });
});
