...
describe("person refresh request-id diagnostics wiring", () => {
  ...
  it("records Getty local prefetch metadata before sending the refresh request", () => {
    ...
    expect(pageContents).toContain("Starting local Getty discovery prefetch before backend refresh.");
    ...
    expect(pageContents).not.toContain("Getty remote probe is blocked or unavailable. Falling back to local Getty discovery.");
