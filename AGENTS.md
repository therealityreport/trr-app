...
Canonical repo policy for `TRR-APP`. Read `../AGENTS.md` first for workspace rules; use this file only for app-local rules and validation.
...
- Server-side data access stays on the `pg` helpers in `apps/web/src/lib/server/postgres.ts`; reserve `@supabase/supabase-js` for auth flows in `apps/web/src/lib/server/auth.ts`.
...
- When admin UI, route behavior, or data rendering changes, run targeted browser validation with `chrome-devtools` when available or the managed-Chrome scripts referenced in `../AGENTS.md`.
